            ...
            <div class="col-md-9">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="GET" action="{{ url('/tickets') }}" accept-charset="UTF-8" class="form-inline my-2 my-lg-0 float-right" role="search">
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <thead>
                                    <tr>
                                        <th>#</th><th>Iva</th><th>Total</th><th>IdCuenta</th><th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                @foreach($tickets as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->iva }}</td><td>{{ $item->total }}</td><td>{{ $item->idCuenta }}</td>
                                        <td>
                                            <a href="{{ url('/tickets/' . $item->id) }}" title="View Ticket"><button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i> View</button></a>
                                            ...
                                            <form method="POST" action="{{ url('/tickets' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
